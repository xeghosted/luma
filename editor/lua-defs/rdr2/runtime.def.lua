---@meta
-- LuaLS definitions for the RDR2Lua runtime API. Hand-written: unlike the
...
---Write to rdr2lua.log and an on-screen notification. Every argument is
...
---Write to rdr2lua.log AND raise a PS4 on-screen notification. This is the
...
-- /data/rdr2lua/resources with an fxmanifest.lua). A plain script in
-- /data/rdr2lua/scripts gets the globals above and `on_tick`, but not these:
...
---is one: a line typed in the editor's RDR2Lua terminal reaches `handler`.
